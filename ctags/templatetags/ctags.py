...
from ctags.utils import LINEAR
    ...
    def render(self, context):
        ...
        if model is None:
            raise TemplateSyntaxError(
                _('ctags_for_model tag was given an invalid model: %s') %
                self.model)
        context[self.context_var] = CTag.objects.usage_for_model(
...
class TagCloudForModelNode(Node):
    ...
    def render(self, context):
        ...
        if model is None:
            raise TemplateSyntaxError(
                _('ctag_cloud_for_model tag was given an invalid model: %s') %
                self.model)
        context[self.context_var] = CTag.objects.cloud_for_model(
            model, **self.kwargs)
        return ''
...
class TagsForObjectNode(Node):
    ...
    def render(self, context):
        context[self.context_var] = \
            CTag.objects.get_for_object(self.obj.resolve(context))
        return ''
...
class TaggedObjectsNode(Node):
    ...
    def render(self, context):
        ...
        if model is None:
            raise TemplateSyntaxError(
                _('ctagged_objects tag was given an invalid model: %s') %
                self.model)
        context[self.context_var] = CTaggedItem.objects.get_by_model(
            model, self.tag.resolve(context))
        return ''


def do_ctags_for_model(parser, token):
    """
    Retrieves a list of ``CTag`` objects associated with a given model
    and stores them in a context variable.

    Usage::

       {% ctags_for_model [model] as [varname] %}

    The model is specified in ``[appname].[modelname]`` format.

    Extended usage::

       {% ctags_for_model [model] as [varname] with counts %}
    ...
    Examples::
       ...
       {% ctags_for_model products.Widget as widget_tags %}
       {% ctags_for_model products.Widget as widget_tags with counts %}
...
def do_ctag_cloud_for_model(parser, token):
    """
    Retrieves a list of ``CTag`` objects for a given model, with tag
    cloud attributes set, and stores them in a context variable.

    Usage::

       {% ctag_cloud_for_model [model] as [varname] %}

    The model is specified in ``[appname].[modelname]`` format.

    Extended usage::

       {% ctag_cloud_for_model [model] as [varname] with [options] %}
    ...
    Examples::
       ...
       {% ctag_cloud_for_model products.Widget as widget_tags %}
       {% ctag_cloud_for_model products.Widget as widget_tags
...
def do_ctags_for_object(parser, token):
    """
    Retrieves a list of ``CTag`` objects associated with an object and
    stores them in a context variable.

    Usage::

       {% ctags_for_object [object] as [varname] %}

    Example::

        {% ctags_for_object foo_object as tag_list %}
    """
    bits = token.contents.split()
    if len(bits) != 4:
        raise TemplateSyntaxError(
...
def do_ctagged_objects(parser, token):
    """
    Retrieves a list of instances of a given model which are tagged with
    a given ``CTag`` and stores them in a context variable.

    Usage::

       {% ctagged_objects [tag] in [model] as [varname] %}

    The model is specified in ``[appname].[modelname]`` format.

    The tag must be an instance of a ``CTag``, not the name of a tag.

    Example::

        {% ctagged_objects comedy_tag in tv.Show as comedies %}
...
register.tag('ctags_for_model', do_ctags_for_model)
register.tag('ctag_cloud_for_model', do_ctag_cloud_for_model)
register.tag('ctags_for_object', do_ctags_for_object)
register.tag('ctagged_objects', do_ctagged_objects)
